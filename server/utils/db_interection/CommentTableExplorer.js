...
class CommentTableExplorer {
  ...
  async createComment() {
    ...
    const newComment = await db.query(
      "INSERT INTO comment (content, post_id, forum_id, user_id, publication_date, publication_time) values($1, $2, $3, $4, $5, $6) RETURNING *",
      [this.content, this.postId, this.forumId, this.userId,date.rows[0].to_char, time.rows[0].localtimestamp]
    );
  ...
  async getComment() {
    const comment = await db.query(`SELECT * FROM comment WHERE id = $1`, [this.id])
    return comment.rows
  }
